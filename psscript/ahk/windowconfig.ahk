...
; =========================
; 2. Win+Tab → Toggle desktop (your original code)
; =========================
#Space::
    toggle := !toggle
    if (toggle)
...
; Prevent CapsLock from toggling when used with Win
#CapsLock up::
return
...
F4:: SendInput, {Alt down}{Alt up}hvst
...
^6::
    ...
    {
        SendInput {Alt Down}h{Alt Up}  ; Tidak memicu hotkey !h kamu
        Sleep 40
        SendInput fs                  ; Decrease Font Size
    }
return
...
#1::
xl := ComObjActive("Excel.Application")
xl.Selection.Style := "Style1"
return
#2::
xl := ComObjActive("Excel.Application")
xl.Selection.Style := "Style2"
return
#3::
xl := ComObjActive("Excel.Application")
xl.Selection.Style := "Style3"
return
#4::
Try {
    xl := ComObjActive("Excel.Application")
    ...
    ; Bottom border — thick + grey
    rng.Borders(9).LineStyle := 1       ; xlContinuous
    rng.Borders(9).Weight    := 4       ; xlThick
    rng.Borders(9).Color     := 0xC0C0C0

    ; Left border
    rng.Borders(7).LineStyle := 1
    rng.Borders(7).Weight    := 2

    ; Top border
    rng.Borders(8).LineStyle := 1
    rng.Borders(8).Weight    := 2

    ; Right border
    rng.Borders(10).LineStyle := 3
    rng.Borders(10).Weight    := 4
    rng.Borders(10).Color     := 0xC0C0C0
...
$`::SendInput, ^c
+Del::
    ; Delete entire active row in Excel
    SendInput, ^-{r}{enter}                              ; Ctrl + -  (delete)
return

^Del::
MsgBox, 4, Confirm Clear, Are you sure you want to CLEAR all content in selected cells?`n`nThis action cannot be undone!,  ; 4 = Yes/No
IfMsgBox, No
    return    ; cancel

Try {
    xl := ComObjActive("Excel.Application")
    xl.Selection.Clear()   ; clear selected cells
} Catch e {
